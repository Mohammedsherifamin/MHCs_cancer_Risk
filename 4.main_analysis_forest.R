...
results <- results %>%
  mutate(p_value = ifelse(p_value < 0.001, "<0.001*", 
                          ifelse(p_value < 0.05, paste0(round(p_value, 3), "*"), 
                                 round(p_value, 3))))
...
# Plot the dataframe
f_mhc_all <- forest(
  results[, c("Model"," ", "HR (95% CI)", "p_value")],
  est = results$est,
  lower = results$low, 
  upper = results$hi,
  ci_column = 2,
...
f_mhc_all <- add_text(
  f_mhc_all, 
  text = "
HR: Hazard Ratio; CI: Confidence Interval; PTSD: Post-Traumatic Stress Disorders.\nModel 1 adjusted for sociodemographic factors; \nModel 2 adjusted for sociodemographic and health-related factors;\nModel 3 adjusted for sociodemographic factors, health-related, and lifestyle factors.\n*Significance level set at alpha = 0.05.",
  row = nrow(results) + 3, 
  col = 1:3, 
  just = "left",
  gp = gpar(fontsize = 7.8, col = "black", fontface= 3, fontfamily = "Helvetica"),
  padding = unit(4, "mm")
)
...
ggplot2::ggsave(filename = "output/new/Figure2-new.pdf", plot = f_mhc_all,
                width = 7.2 , height = 7.5, units = "in")



# Sex stratified forest ---------------------------------------------------

extract_cox_data <- function(model, coef_name) {
  summary_model <- summary(model)
  est <- exp(coef(model)[coef_name])
  ci <- exp(confint(model)[coef_name, ])
  se <- (log(ci[2]) - log(est)) / 1.96
  p_value <- summary_model$coefficients[coef_name, "Pr(>|z|)"]
...
# Calculate the indices for each row
indices <- seq_len(nrow(results)) - 1  # Adjust for 0-indexing in R
...
ggplot2::ggsave(filename = "output/new/Supplementary Figure 1-new.pdf", plot = f_mhc_sex,
...
ggplot2::ggsave(filename = "output/new/Figure 3-new.pdf", plot = f_dep,
                width = 6.5, height = 9.9, units = "in")
...
ggplot2::ggsave(filename = "output/new/Figure 4-new.pdf", plot = f_anx,
                width = 6.5, height = 9.9, units = "in")
